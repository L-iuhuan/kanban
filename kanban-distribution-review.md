...
> 来源:另一会话(分发前全链路终审) | 日期:2026-08-18 | 状态:**已执行并全链路验证通过(2026-08-18,见第十节)**;仅第四节手册话术项与真实共享盘投产发布待做
...
## 十、施工与全链路验证记录(2026-08-18,本轮已完成)

**施工**:P0 便携 Python(portable-python 669MB,组装自检 `PORTABLE_OK`,robocopy 解释器+venv site-packages 重定位);运行器 v0.3.9(venv_python 便携优先/sync_code 二段 robocopy 同步 python\/setup_env 便携短路(直接判便携路径,不误伤 .venv 半残自愈)/RunEvent::Exit 孤儿进程 taskkill /T /F/autoRepairEnv 两处自愈);发布脚本([1/4]~[4/4] 四段:代码+deps.txt 生成+python 通道+壳子,UTF-8 BOM 保持);P1-3 前置校验(流水线仓库 e42c52c)。

**全链路验证(模拟全新机器,数据目录整体换新+预置 config)**:
| 检查项 | 结果 |
|---|---|
| 首启自举 | ✅ 33s 同步完 code+517MB python;全程无 venv/pip 动作 |
| 便携解释器本地导入 | ✅ `LOCAL_PORTABLE_OK`(十依赖全量) |
| deps.txt | ✅ 10 条,UTF-8 无 BOM,映射正确 |
| 流水线入口(无数据) | ✅ 中文报错"未找到原始Excel"引导,退出码 1 |
| P1-3 skip-processing 无缓存 | ✅ 新前置校验中文拦截,退出码 1 |
| 回落路径 | ✅ 真实目录(.venv)恢复后应用正常启动 |
| P1-1/P1-2 | ✅ 运行时抽查通过(2026-08-18 用户实测):流水线运行中直接关窗 → 重开再拖入文件正常运行,无互踩;且能复用之前的处理缓存。孤儿回收+自愈+缓存复用全链路行为符合预期 |
| 发布脚本端到端 | ✅ 真跑四段全通过(本节顶部模拟盘即其产物) |

**遗留**:第四节手册话术项(文档级);真实共享盘投产发布(portable-python ~700MB 首推+0.3.9 壳子)待择机执行;P1-1/P1-2 运行时抽查。
