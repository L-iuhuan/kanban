...
给测试人员(纯小白)用的桌面应用:**双击即用,自动从共享盘拉取最新看板代码,选 Excel（或点「从共享盘获取最新数据」一键拉取）一键生成看板,看进度日志,直接打开看板和结果文件**。Tauri 2 + Rust 壳 + Python 流水线。
...
数据共享目录(独立于代码共享目录,设置里「数据文件共享目录」可配,留空用内置默认):
...
└─ %LOCALAPPDATA%\KanbanRunner\                ← 数据目录(exe 装哪都行)
    ...
    ├─ data\          ← 数据缓存(共享盘拉取的 Excel 落这里)
    ├─ .venv\         ← Python 环境(便携模式不可用时回落)
